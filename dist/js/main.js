...
(() => {
  ...
  window.addEventListener("keydown", e => {
    if (Observatoire.estOuvert()) return;
    if (!panneauCommandes.hidden && e.key === "Escape") return;
  ...
  const panneauCommandes = document.getElementById("panneau-commandes");

  function mettreAJourEtatOptions() {
    btnSon.querySelector(".option-icone").textContent = sonCoupe ? "🔇" : "🔊";
    btnSon.querySelector("[data-son-etat]").textContent = sonCoupe ? "Coupé" : "Actif";
    btnCalme.querySelector("[data-calme-etat]").textContent = modeCalme ? "Actif" : "Inactif";
  }

  function ouvrirMenu() {
    panneauCommandes.hidden = false;
    btnMenu.setAttribute("aria-expanded", "true");
  }

  function fermerMenu(rendreFocus = false) {
    if (panneauCommandes.hidden) return;
    panneauCommandes.hidden = true;
    btnMenu.setAttribute("aria-expanded", "false");
    if (rendreFocus) btnMenu.focus();
  }

  btnMenu.addEventListener("click", () => {
    if (panneauCommandes.hidden) ouvrirMenu();
    else fermerMenu();
  });
  btnFermerMenu.addEventListener("click", () => fermerMenu(true));
  document.addEventListener("pointerdown", e => {
    if (!panneauCommandes.hidden && !panneauCommandes.contains(e.target) && !btnMenu.contains(e.target)) {
      fermerMenu();
    }
  });
  document.addEventListener("keydown", e => {
    if (e.key === "Escape" && !panneauCommandes.hidden) {
      e.preventDefault();
      fermerMenu(true);
    }
  });
